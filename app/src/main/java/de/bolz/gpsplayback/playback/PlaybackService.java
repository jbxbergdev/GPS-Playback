...
public class PlaybackService extends Service {
    ...
    private void start() {
        ...
        LocationSource locationSource = new JsonLocationSource(LocationManager.GPS_PROVIDER, new File(Environment.getExternalStorageDirectory(), "track.json"));
        playbackEngine.startPlayback(locationSource);
        playbackEngine.getPlaybackState().observeForever(playbackStateExceptionResource -> {
            if (playbackStateExceptionResource.data == PlaybackEngine.PlaybackState.STOPPED) {
                stopMe();
            }
        });
    }
